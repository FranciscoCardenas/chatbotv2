...
class MetaProvider extends ProviderClass {
    ...
    sendMessage = async (number, message, { options }) => {
        if (options?.buttons?.length) return this.sendButtons(number.replace('521', '52'), message, options.buttons)
        if (options?.media) return this.sendMedia(number.replace('521', '52'), message, options.media)

        this.sendtext(number.replace('521', '52'), message);
    }
}
